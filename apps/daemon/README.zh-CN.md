...
- **Conversation model 快照** - 创建 conversation 时会持久化 `conversation.model`，优先级为 `agents.list[].model`（或匹配到的 legacy `routing.agents.<id>.model`）-> `agents.defaults.model` -> 适配器/环境回退（`OPENCLAW_MODEL`、`OPENAI_MODEL`、`MODEL`）；发送消息时优先使用持久化 model，只有旧会话 `model=null` 时才走适配器/环境回退
